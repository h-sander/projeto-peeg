...
videoUrl = `https://www.googleapis.com/youtube/v3/videos?part=snippet&id=${idVideo[`${nVideo}`]}&key=AIzaSyAlz0tMpkgRIQsZcWK9F7iz3vWpMziyNzg`;

// Adds quizz URL to quizz iframe and forces it to reload
document.getElementById('quizz').contentDocument.location.reload(true);
document.getElementById('quizz').src = quizUrl[nVideo];

document.getElementById("course-name").innerHTML += nomeCurso;
const videoConfig = '&autoplay=1&rel=0';
...
document.getElementById('vid-frame').src += idVideo[nVideo]; // ADICIONA O ID DO VÍDEO AO IFRAME NO HTML
...
$("#newFile").click(function() { 
  var blob = new Blob([`const matAula = [${matAulaPass}]; const quizUrl = [${quizUrl}]; const idVideo = [${idVideo}]; const titleVideo = [${titleVideo}]; const playlistUrl = "${playlistUrl}"; const nomeCurso = "${nomeCurso}"; const moodleUrl = "${moodleUrl}"; let varRead = "lido";`], {type: "text/javascript"});
...
Http.onreadystatechange = function () {
  if(Http.readyState === XMLHttpRequest.DONE && Http.status === 200) {
    ...
    const desc = JSON.stringify(j.items[0].snippet.description);
